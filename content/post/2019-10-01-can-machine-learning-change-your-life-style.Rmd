...
We are developing new technologies at lightning speed. We can now analyse genes from a single cell which was not even possible couple of years back, and extract the knowledge hidden in rare genes or rare cell types. Sensors collect data from patients e.g. in diabetic patients, we started to pool healthcare information which accumulate during the use of drugs (e.g. [OMOP common data model](https://www.ohdsi.org/data-standardization/the-common-data-model/) that is called real world evidence. **The data that we have access now is enormous and is growing everyday.** 
...
* **identify patients groups which benefit most from existing drugs**


**Let's look at an example** and try to help doctors in diagnosing patients.

First part of data science process is problem formulation.
...
I look at what atomic data types my variables has. I see that the outcome variable is represented as an integer. I will keep this in mind because many machine learning models will accept the binary outcome as factor data type.

Next, what catches my attention is **unexpected zero values in Insulin**. See below.
```{r head}
...
I will change the data format so that I can plot all the variables in different facets. melt() function from reshape2 package can create a tall version of my data. **This function will collect all variable names in one column and corresponding values in the next column.** This data structure will allow me to plot all variables together.
...
Multiple variavles have peaks at zero. E.g. peaks at zero of Skin Thickness and Insulin is obvious here. 
...
I am selecting only variables from 2 to 6 because they can't be zero. Let's count number of zeros on those variables.

How to count zeros in each column using dplyr?

Approach 1: Using which and []
...
As a bonus, I can recommend a shorter and faster approach
Approach 2: Using dplyr functions and **summarise_each()**

```{r}
start <- Sys.time()
gg %>% group_by(variable) %>% summarise_each(funs(sum(.==0)))
end <- Sys.time()
with_dplyr <- end-start
```

Using dplyr is easier and faster. For loop: ```with_loop```, Dplyr: ```with_dplyr```
...
For instance, **374** rows of Insulin are zero. Other variables also contain zeros. Somethin is wrong. It is impossible to have Blood Pressure or Glucose levels at 0. It is unlikely that those are simply entry mistakes. It seems missing values are filled with **zeros** in the data collection phase. 
