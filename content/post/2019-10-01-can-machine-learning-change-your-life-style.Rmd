...
For instance, in Insulin **374** values are zero. Other variables also contain zeros. It is impossible to have Blood Pressure or Glucose levels at 0. It is unlikely that those are simply entry mistakes. It seems missing values are filled with **zeros** in the data collection phase. 
...
Before going into any complicated model starting with a simple mode is a good idea. It might do surprisingly well and will give us more insights about the data. 
...
We will build a confusion matrix to calculate how accurate our model is in this particular random train/test split and at 0.5 thereshold level. 
...
```{r multi split function test, cache=TRUE, message=FALSE, warning=FALSE}
...
Great. We have a good estimate of our linear model performance. 
