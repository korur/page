---
title: 'Genomics at superresolution: Advances in sequencing technologies'
author: ''
date: '2019-09-17'
slug: genomics-at-superresolution-advances-in-sequencing-technologies
categories: []
tags:
  - scRNA seq
  - Seurat
...
Advances in **microfluidic technologies** enabled us barcoding single cells in lipid droplets and to **resolve genomes of individual cells** from a sequencing mixture (e.g, [10X Genomics](https://www.10xgenomics.com/)). **Single cell RNA sequencing (scRNAseq)** offers great opportunities to discover **rare cell populations**  and genes that are specifically expressed in those. There is a high interest and the list of publications growing daily.  

* You might be a scientist in a biotech exploring your novel targets and those might be a great source to **retrieve specific information.**

[Seurat](https://cran.r-project.org/web/packages/Seurat/index.html) package is a great tool to digging into single cell datasets. Learning it will open you access beyond what is shown in the publication. You can find many tutorials in their [website](https://satijalab.org/seurat/).
...
* **Pulmonary fibrosis** is a progressive scarring of the lung tissue leading to death within 2-5 years.
* Current therapies does not increase the survival but many Biotechs are investigating novel therapies. 

The paper utilized 8 healthy and 8 diseased lung samples. Here, I picked up the data from a patient with Polymyositis associated intersitial lung disease to apply main functions. 
...
Here the data comes as an **h5 file.** But no worries, we can handle that with **Read10X_h5** function in Seurat.
```{r  warning=FALSE}
...
by using the `scale data` function
...
# cells that arenormally cycling but in different phases of cell cycle might appear 
...
Each gene creates another dimension in our dataset, but most of those does not play a role in differenting subgroups of cells. Principal components analyses (PCA) reduces the dimensions of our data into main compoments which explains most of the variation. 
...
### What is the 'dimensionality' of out data

For clustering analyses we will choose the principal compoments which explain most of the variation in our data. `Elbow plot` uses a ranking of principle components based on the percentage explained by each one. 

```{r elbow_plot, fig.height=6, fig.width=10}
ElbowPlot(object = Fib)
```

We did not get a clear elbow shape here, but after 10th principal compoment additional dimensions does not explain big amount of the variance. So we will use first 10 dimensions for the subsequent analyses.

### Cluster the cells

A graph-based clustering approach will be performed, building upon initial strategies in ([Macosko *et al*](http://www.cell.com/abstract/S0092-8674(15)00549-8)).
...
# According to the table above I generated a cluster id vector to assign cell type
# to each cluster. I have a group NA which I could not assign a cell type, 
# which is probably a technical artifact from cell cycle regression since the top genes
# expressed in this population are cell cycle related.
# I also found 3 types of macrophages and 2 types of mast cells in this patient. 
...
### What cells `Biotech` Companies are targeting in lung?

#### Fibrinogen

```{r Fibrinogen}
#### Fibrinogens lead compound pamrevlumab blocks CTGF. Let's have a look which cells produce it.  
...
# Galapagos GLPG1205 has entered Phase 2 clinical trials and it targets GPR84. 
...
# Let's look at the epxression of a LPAR1
VlnPlot(Fib, features = c("LPAR1"))

FeaturePlot(Fib, features =c("LPAR1"))
```
...
VEGFA, PDGFA is highly expressed in multiple types of lung microenvironment.
